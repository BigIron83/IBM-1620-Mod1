...
from ibm1620_parts.model.repositories import (
    fetch_raw_part_candidates,
    fetch_reviewed_part_candidates,
    get_connection,
)
...
def export_reviewed_parts_csv(db_path: Path, output_path: Path) -> None:
    output_path.parent.mkdir(parents=True, exist_ok=True)
    with get_connection(db_path) as conn:
        rows = fetch_reviewed_part_candidates(conn)
